...
SubMenu_Items::SubMenu_Items()
{
	...
	_Titulo->setString("MENU ITEMS");
	_Banner->setTexture(HEADER_MENUITEMS);
...
void SubMenu_Items::update(sf::RenderWindow& _Ventana)
{
...
void SubMenu_Items::inicializarBotones()
{
	//0 Agregar item
	_Botones[0].setbothPos(80.0f, 280.0f);
	_Botones[0].setBtnType(BTN_ADD_ITEM);
	_Botones[0].setTextoString("Agregar Items");
	//1 Mostrar items

	//if (itemsaMostrar == 0) _Botones[1].setBtnEstado(DESHABILITADO);
	//else _Botones[1].setBtnEstado(NINGUNO);

	_Botones[1].setbothPos(480.0f, 280.0f);
	_Botones[1].setBtnType(BTN_LIST_ITEM);
	_Botones[1].setTextoString("Mostrar Items");
