#include "Items_Alta.h"
#include "Singleton.h"
#include "SubMenu_Items.h"

extern sf::Font* _FUENTE;

void Items_Alta::inicializarBotones()
{
}

void Items_Alta::verificacionInput()
{
	std::string algo;
	char auxc[30] = {};
	float auxf = 0;
...
bool Items_Alta::validarChecks(bool checks[], int tam)
{
	for (int i = 0; i < tam; i++) {
		if (checks[i] != true) {
			return false;
...
bool Items_Alta::validarNum(std::string& num)
{
...
Items_Alta::Items_Alta()
{
	...
	_Titulo->setString("AGREGAR ITEMS");
...
void Items_Alta::process_event(const sf::Event& e)
{
...
void Items_Alta::update(sf::RenderWindow& _Ventana)
{
...
void Items_Alta::dibujarEnVentana(sf::RenderWindow& _Ventana)
{
	//DIBUJADOS
